...
export default function OutputPanel({ snapshots, onSnapshotAdd, rhythmSettings }) {
  ...
  // Generate note events from the selected snapshot + rhythm settings
  ...
  // Apply tempo scaling — faster tempoScale → shorter durations
  ...
  // Destroy AudioEngine when unmounting
    ...
    <div className="space-y-6">
      ...
      {!importedEvents ? (
        ...
        <div className="flex items-center gap-3 text-sm">
          <span className="text-slate-500">Showing imported file ({noteEvents.length} events)</span>
          <button
            onClick={() => setImportedEvents(null)}
      ...
      {/* Folded score display */}
      <FoldedScore noteEvents={noteEvents} baseHz={BASE_HZ} />
      ...
      {noteEvents.length === 0 && snapshots.length === 0 && !importedEvents && (
        <p className="text-sm text-slate-400 text-center py-8">
          Generate a composition in the Compose step and save a snapshot, or import a CSound .txt file.
        </p>
      )}
    </div>
  )
